            ...
            delayedA {
              before (KNOCKOUT, self) {
                if ((ef as Knockout).byDamageFromAttack && bg.currentTurn == self.owner.opposite && self.owner.pbg.deck && confirm("Use Final Wish?", self.owner)) {
                  powerUsed()
                  self.owner.pbg.deck.select("Search your deck for a card",{true}, self.owner).moveTo(hidden:true, self.owner.pbg.hand)
                  shuffleOppDeck()
                }
              }
            }
        ...
        return evolution (this, from:"Nidorina", hp:HP120, type:PSYCHIC, retreatCost:3) {
          ...
          pokeBody "Maternal Comfort", {
            ...
            delayedA (anytime:true) {
              def lastExecId = null
              before BEGIN_TURN, {
                if (lastExecId != e.executionId && bg.em().retrieveObject("Maternal_Comfort" + self.owner) != ef.oldTurnCount && self.owner.pbg.all.any{it.numberOfDamageCounters} ) {
                  bc "$thisAbility activates"
                  ...
                  bg.em().storeObject("Maternal_Comfort" + self.owner, ef.oldTurnCount)
                }
              }
            }
          }
      ...
      case LEAFEON_45:
        return evolution (this, from:"Eevee", hp:HP090, type:GRASS, retreatCost:2) {
          ...
          pokeBody "Energy Refresh", {
            ...
            delayedA {
              after ATTACH_ENERGY, {
                if (ef.reason == PLAY_FROM_HAND && self.active && ef.resolvedTarget.owner == self.owner) {
                  bc "$thisAbility activates."
